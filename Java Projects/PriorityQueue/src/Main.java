...
public class Main {
    ...
    size()------> Kuyruğun içindeki eleman sayısını döner.
        */
        ...
        queue.offer(new Player("Mehmet",3));
        queue.offer(new Player("Enes",100));
        
        while(!queue.isEmpty()) {
            
            System.out.println("Eleman Çıkarılıyor : " + queue.poll()); // buranın çalışması için toString in override edilmesi gerekiyor
        }
        /*
        queue.offer(5);
        queue.offer(1);
        ...
        while(!queue.isEmpty()){
            ...
            // tek tek elemanları çıkartarak görebiliyoruz
        }
        /*for(Integer i : queue){
            System.out.println(i);
        }*/ 
        // Biz sıralamayı direkt for each döngüsüyle göremeyiz (sırasız veriyor)
